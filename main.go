...
const (
	...
	KmHInMsec                        = 0.277777778 // коэффициент для перевода км/ч в м/с
...
func (r Running) Calories() float64 {
	speed := r.meanSpeed()
	timeInHours := r.Duration.Minutes() / MinInHours
	return ((CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * r.Weight / MInKm * timeInHours * MinInHours)
}
...
func (w Walking) Calories() float64 {
	...
	timeInHours := w.Duration.Minutes() / MinInHours
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/height)*CaloriesSpeedHeightMultiplier*w.Weight) * timeInHours * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	distance := s.LengthPool * float64(s.CountPool) / MInKm
...
func (s Swimming) Calories() float64 {
	speed := s.meanSpeed()
	timeInHours := s.Duration.Minutes() / MinInHours
	return (speed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * timeInHours
}
